...
\fontsize{6}{2}\selectfont
  ...
  \section*{Constants}
  \begin{gather}
    c = \lambda \nu = \qty{2.997925e8}{\meter\per\second} \tag{Speed of Light in Free Space} \\
  ...
  \begin{gather}
    ...
    s^2 = (c\Delta t)^2 - \left\Vert{\Delta\vec{x}}\right\Vert^2 \implies \left(\mathrm{d}S\right)^2 = (c\mathrm{d}t)^2 - \left\Vert{\mathrm{d}\vec{x}}\right\Vert^2 \tag{Spacetime Interval} \\
  ...
  \begin{gather}
    ...
    \frac{1}{\lambda} = R_{\ce{H}} \left( \frac{1}{n_1^2} - \frac{1}{n_2^2} \right) \tag{Rydberg Formula for a \ce{H} electron} \\
    R_{\ce{H}} \coloneq -\frac{m_e e^4}{8h^3\varepsilon_0^3c}\tag{Rydberg Constant for a \ce{H} electron}
  \end{gather}
  ...
  \begin{gather}
    ...
    V(x_0, t) = \left.{\frac{{\partial}D(x,t)}{{\partial}t}}\right\vert_{x=x_0} \hspace{-4ex}= -\omega A \cos{\left(\omega t + \varphi'\right)} \tag{SWD particle velocity history graph}
  \end{gather}
